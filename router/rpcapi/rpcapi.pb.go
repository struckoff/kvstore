...
func NewRPCCapacityClient(cc grpc.ClientConnInterface) RPCCapacityClient {
	return &rPCCapacityClient{cc}
}

func (c *rPCCapacityClient) RPCGet(ctx context.Context, in *Empty, opts ...grpc.CallOption) (*Capacity, error) {
	out := new(Capacity)
	err := c.cc.Invoke(ctx, "/rpcapi.RPCCapacity/RPCGet", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (*UnimplementedRPCCapacityServer) RPCGet(ctx context.Context, req *Empty) (*Capacity, error) {
	return nil, status.Errorf(codes.Unimplemented, "method RPCGet not implemented")
}

func RegisterRPCCapacityServer(s *grpc.Server, srv RPCCapacityServer) {
	s.RegisterService(&_RPCCapacity_serviceDesc, srv)
}

func _RPCCapacity_RPCGet_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(Empty)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(RPCCapacityServer).RPCGet(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/rpcapi.RPCCapacity/RPCGet",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RPCCapacityServer).RPCGet(ctx, req.(*Empty))
	}
	return interceptor(ctx, in, info, handler)
}

var _RPCCapacity_serviceDesc = grpc.ServiceDesc{
	ServiceName: "rpcapi.RPCCapacity",
	HandlerType: (*RPCCapacityServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "RPCGet",
			Handler:    _RPCCapacity_RPCGet_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "rpcapi.proto",
